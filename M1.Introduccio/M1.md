...
De segur que molts de vosaltres ja heu sentit parlar de la inteligència artificial y de tot allò que pot fer, alguns ja heu començat a utiltizar-lo en el vostre dia a dia i cal distingir alguns conceptes sobre el que és la IA. En aquest curs tracterem de fer-vos una introducció sobre les diferents ferramentes que existeixen i com traure-li's profit. 

Cal tenir en compte que es tracta d'un curs d'iniciació i és possible que vos sentiu abrumats de tota la informació que aneu a veure, òbvimanet per la durada del curs no anem a poder aprofundir en molts de les utilitats que vos presentarem, això ha vos ho deixarem a vosaltres.

Els mòduls seran breus però intensos, tractarem d'ajudar-vos en tots els dubtess que se vos plantegen, però nosaltres també hem decidit fer ús de les diferents ferramentes d'IA per a fer el curs, vos presentarem el propmpt[^1] i la resposta que ens donarà, tractarem de limitar l'extensió al que realment necessitiu. Les resposte vos dient que estaran retocades ja que la resposta que dona sempre qualsevol plataforma sempre ha de retocar-se per a que siga el més idònia possible al que desitges. Vos ho presentarem amb la següent icona.

[^1]: Prompt, és el text que escrius a la paltaforma per a que interprete el que realment necessites. Entrarem amb més detall a la propera unitat.
...
# Que és i què no és la inteligència artificial?

Podem pensar que tot el que fem a l'ordinador té a veure amb la inteligència artificial i òbviament no és així, els ordiandors utilitzen algoritmes amb llenguatges de programació per a poder automatitzar tasques o realitzar programes. 

Aqui tens un exemple de diagrama de fluxe senzill:
...
Dins de la mateixa inteligència artifial ens podem trobar diferents categories que anirem vegent al llarg dels propers anys.
...
Els Models de Llenguatge a Gran Escala (MLL, per les seues sigles en anglés, Large Language Models) són models d'intel·ligència artificial que han sigut entrenats amb enormes quantitats de dades textuals per a aprendre patrons, estructures i representacions del llenguatge natural. Estos models són capaços de realitzar tasques relacionades amb el processament del llenguatge, com entendre el significat de frases, generar text coherent i respondre preguntes.

Exemples de MLL inclouen GPT-3 (Generative Pre-trained Transformer 3) d'OpenAI, BERT (Bidirectional Encoder Representations from Transformers) de Google, i T5 (Text-to-Text Transfer Transformer) de Google.
...
En resum, Quick, Draw! combina la diversió d'un joc en línia amb la recopilació de dades per a millorar els models d'intel·ligència artificial de reconeixement de patrons. Els usuaris contribueixen a la millora de la tecnologia mentre participen en una experiència interactiva i creativa.
